//
//  settingMemViewController.m
//  BLT-Spo2Monitor
//
//  Created by luteng on 16/7/18.
...
@interface settingMemViewController ()<UITableViewDelegate, UITableViewDataSource,UITextFieldDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,MBProgressHUDDelegate,UIPickerViewDelegate, UIPickerViewDataSource,OMEDataServiceDelegate>
...
@implementation settingMemViewController
...
-(void)viewDidLoad
{
    ...
    [customLab setText:NSLocalizedString(@"Family_EditUser", @"编辑用户")];
    ...
    _saveButton = [[UIButton alloc]initWithFrame:CGRectMake(10, self.tableView.frame.origin.y+self.tableView.frame.size.height, self.view.frame.size.width-10*2, 44)];
    ...
    [_saveButton setTitle:NSLocalizedString(@"Family_Update", @"更新") forState:UIControlStateNormal];
...
-(UIDatePicker*)datePicker
{
    if(!_datePicker)
    {
        ...
        _datePicker.date = self.selectedUser.birthday;//[@"1980-01-01" stringWihtDateFormat:@"yyyy-MM-dd"];
...
-(OMEImageView*)userImageView
{
    if(!_userImageView)
    {
        ...
        if (_selectedUser.imageName) {
            _userImageView.imageName = _selectedUser.imageName;
            _userImageView.memberId = _selectedUser.memberId;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        switch (indexPath.row) {
            case 0:
                titleStr =NSLocalizedString(@"Family_Avatar",@"Avatar");
                [cell.contentView addSubview:self.userImageView];
                break;
            case 1:
                titleStr =NSLocalizedString(@"Family_Name",@"Name");
            ...
            case 2:
            {
                titleStr =NSLocalizedString(@"Family_Gender",@"Gender");
...
-(void)saveAction
{
    ...
    self.progressHUD.labelText = NSLocalizedString(@"Family_Updating", @"正在更新");
    ...
    if ([loginedArray count]) {
        ...
        /*
        NSString *string = [_imageData base64Encoding];
        NSLog(@"压缩图片前大小＝%d", string.length);
        ...
        NSData *adata = [[NSData alloc] initWithBytes:buf length:blen];
        NSLog(@"压缩图片后大小＝%d", [adata base64Encoding].length);
         */
        //
        ...
        NSDictionary *userInfoDic = [NSDictionary dictionaryWithObjectsAndKeys:
                                     @"UpdateMemInfo", @"key",
                                     @(idNum), @"accountId",
                                     @(self.selectedUser.familyId),@"memberId",
                                     key, @"clientKey",
                                     memeInfo, @"memInfo",
                                     nil];
        ...
        [_dataService requestOMESetFamilyByMemberId:idNum Key:key familyId:self.selectedUser.familyId json:jsonStr];
    }
}
...
{
    [self showWarmTitle:NSLocalizedString(@"Service_UpdateSuccess", @"更新成功") message:nil withImageName:@"37x-Checkmark.png"];
}

- (void)uploadPhotoDidFailed
{
    [self showWarmTitle:NSLocalizedString(@"Service_UpdateFailed", @"更新失败") message:nil withImageName:@"37x-Failed.png"];
}
    ...
    switch (_pickerIndex) {
        ...
        case 1:             //height
            ...
            else//IN
            {
                ...
                _userHeightLabel.text = [NSString stringWithFormat:@"%ld%@",[str integerValue], NSLocalizedString(@"Height_Unit_IN", @"IN")];
            }
            break;
        case 2:             //weight
            if(isKg)
            ...
            else
            {
                ...
                _userWeightLabel.text =[NSString stringWithFormat:@"%ld%@", [str integerValue], NSLocalizedString(@"Weight_Unit_LB", @"LB")];
            }
            break;
        default:
            break;
...
- (void)dataService:(OMEDataService *)dataService didFinishWithData:(NSDictionary *)dataDic
{
    ...
    if ([codeStr intValue]) {
        ...
        NSString *fileName = [data objectForKey:@"avatarDir"];
        ...
        if([userData changeUserWithData:_selectedUser])
        {
